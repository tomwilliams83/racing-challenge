...
// ─── FIREBASE ─────────────────────────────────────────────────────────────────import { useState, useEffect, useCallback, useRef, useMemo } from "react";
import { initializeApp } from "firebase/app";
import { getDatabase, ref, set, get, onValue, off } from "firebase/database";
import {
  getAuth, onAuthStateChanged, signOut,
...
function ProfileScreen({ authUser, onBack, onRejoin }) {
  ...
  const stats = useMemo(() => {
    ...
    history.forEach(ch => {
      ...
      // Only canned challenges (ITV card) count for badges and stats
      const isQualifying = !!ch.isCanned;
                  ...
                  return (
                    <div key={ch.code} style={{ background: "#fff", border: `1.5px solid ${C.border}`,
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 13, color: C.muted }}>
                          {ch.isCanned && <span style={{ color: C.pink, fontWeight: 600, marginRight: 6 }}>📺 Official</span>}
                ...
                <>
                  <button onClick={() => setShowProfile(true)}
                    ...
                    <div style={{ fontSize: 13, color: C.muted }}>
                      ...
                      <span style={{ color: C.blue, marginLeft: 6, fontSize: 12 }}>View profile →</span>
                    </div>
                  </button>
